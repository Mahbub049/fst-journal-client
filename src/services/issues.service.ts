...
type RecentIssuesResponse = { success: boolean; data: Issue[] };
type PublicIssuesResponse = { success: boolean; data: Issue[] };
type SingleIssueResponse = { success: boolean; data: IssueDetailsResponse };
type SingleArticleResponse = { success: boolean; data: ArticleDetailsResponse };
type HomeArticlesResponse = { success: boolean; data: Article[] };
type AdminIssuesResponse = { success: boolean; data: Issue[] };
type AdminIssueResponse = { success: boolean; data: Issue };
...
export const getIssueBySlug = async (slug: string): Promise<IssueDetailsResponse> => {
...
export const getHomeArticles = async (tab: string): Promise<Article[]> => {
  const res = await api.get<HomeArticlesResponse>(`/issues/articles/home?tab=${tab}`);
  return res.data.data || [];
};
...
export const getAdminIssues = async (params?: {
  ...
  const res = await api.get<AdminIssuesResponse>("/issues/admin/all", { params: query });
  return res.data.data || [];
};
...
export const createAdminIssue = async (payload: IssuePayload): Promise<Issue> => {
...
export const reorderAdminIssues = async (issueIds: string[]): Promise<Issue[]> => {
  const res = await api.patch<AdminIssuesResponse>("/issues/admin/reorder", { issueIds });
  return res.data.data || [];
};
